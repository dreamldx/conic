...
- `registry.py::build_plugin_set(config, global_variables=None)` 计算 `resolved_global_variables = {"model": ..., "platform": ..., "shell": ..., "timezone": ..., **(global_variables or {})}`，通过闭包捕获进 `loop_factory`。`shell` 由私有函数 `_detect_shell()` 算出，用 `shellingham.detect_shell()`（第三方库，通过遍历父进程链识别启动 conic 进程的那个交互式 shell，如 `bash`/`zsh`/`powershell`/`cmd`）取 `(name, path)` 中的 `name`；探测失败（`shellingham.ShellDetectionFailure`，例如作为服务/无父 shell 场景运行）时按平台回退：Windows 用 `"cmd.exe"`，其他平台用 `"/bin/sh"`。**注意语义**：这个值是"conic 进程本身跑在哪个 shell 下"，不是"`BashToolPlugin` 执行工具调用时真正用的解释器"——后者固定是 `asyncio.create_subprocess_shell` 的 `shell=True` 语义（Windows 走 `COMSPEC`／通常是 `cmd.exe`，POSIX 固定 `/bin/sh`），跟 conic 进程自身的父 shell 是两回事；`RuntimeSectionPlugin` 早期版本硬编码过 "PowerShell 5.1"，那条提示词本来就是错的，现在换成动态探测值至少不再是写死的错误答案，但如果之后要精确告诉模型"你写的命令实际会被哪个解释器执行"，仍需要单独针对 `BashToolPlugin` 的执行路径去算，而不是依赖这个面向"进程环境"的探测结果；
